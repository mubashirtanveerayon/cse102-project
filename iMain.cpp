...
void iMouse(int button, int state, int mx, int my) {
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN ) {
		...
		if(page == MAIN_MENU_STATE){
			if(mx >= 107 && mx<= 347 && my <= 276 &&my >= 223){ // start button
				page = GAME_STATE;
                PlaySound("click.wav", NULL, SND_ASYNC);
			}else if(mx >= 104 && mx<= 345 && my <= 191 &&my >= 141){ // tutorial button
				page = GAME_STATE;
                initializedTutorial = 1;
                PlaySound("click.wav", NULL, SND_ASYNC);
			}else if(mx >= 108 && mx<= 345 && my <= 110 && my >= 61){ // exit button
            PlaySound("click.wav", NULL, SND_ASYNC);
				exit(0);
			}
		}
...
void shuffleArray(char *str)
{
    ...
    {
        int randomIndex = getRandomIndex(i + 1);
...
void evaluate()
{
    ...
    if (correct)
    {
        ...
        typedWordColor[2] = 0;
        PlaySound("pickupCoin3.wav", NULL, SND_ASYNC);
    }
    else
    {
